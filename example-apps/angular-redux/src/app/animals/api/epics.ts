...
const animalsNotAlreadyFetched = (animalType: AnimalType, state: IAppState): boolean =>
  !(state[animalType] && state[animalType].items && Object.keys(state[animalType].items).length)

const actionIsForCorrectAnimalType = (animalType: AnimalType) => (action: AnimalAPIAction): boolean =>
  action.meta.animalType === animalType

@Injectable()
export class AnimalAPIEpics {
  constructor(private service: AnimalAPIService, private actions: AnimalAPIActions) {}
